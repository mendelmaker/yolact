...
def draw_img(results, img, args, class_color=False, fps=None):
	...
	# nor_classes = (classes - args.visual_thre) / 14.7
	...
	def get_color(j, on_gpu=None):
		...
		if 'cuda' in str(on_gpu):
			color = torch.Tensor(color).to(on_gpu).float() / 255.
		elif 'cpu' in str(on_gpu):
	...
	if not args.hide_mask:
		...
		colors = torch.cat(
			[get_color(j, on_gpu="cuda").view(1, 1, 1, 3) for j in range(num_considered)], dim=0)
